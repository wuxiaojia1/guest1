...
def add_event(request):
    ...
    try:
        ...
        error = 'start_time from error.It must be in YYYY-MM-DD HH:MM:SS format.'
...
def add_guest(request):
    ...
    result = Event.obnects.filter(id=eid)
    if not result:
        return JsonResponse({'status':10022,'message':'event id null'})

    result = Event.objects.get(if=eid).status
    if not result:
...
def user_sign(request):
    ...
    if eid == '' or phone == '':
        return JsonResponse({'startus':10021,'message':'parameter errr'})
